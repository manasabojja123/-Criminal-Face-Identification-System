...
class App:
    def __init__(self):
        ...
        self.cctv_footage_path = None

        # Label for displaying the app name
        ...
        # Treeview for displaying detected people
        self.initialize_treeview()

        # Button for uploading CCTV footage
        self.btn_upload = Button(self.window, text="Upload CCTV Footage", command=self.upload_cctv_footage)
        self.btn_upload.pack()

        # Video capture object
        self.vid = None

        # List to store detected people
        self.detectedPeople = []
    ...
    def upload_cctv_footage(self):
        self.cctv_footage_path = filedialog.askopenfilename()
        if self.cctv_footage_path:
            self.vid = MyVideoCapture(self.cctv_footage_path)
    ...
    def view_detail(self, a):
        ...
        for row in rows:
            ...
            image_path = 'images/user.{}.png'.format(a)
            image = Image.open(image_path)
            image = image.resize((180, 180), Image.ANTIALIAS)
            photo = ImageTk.PhotoImage(image)
            photo_label = Label(self.window, image=photo, width=180, height=180)
            photo_label.image = photo  # To prevent garbage collection
            photo_label.place(x=750, y=450)
        conn.close()
