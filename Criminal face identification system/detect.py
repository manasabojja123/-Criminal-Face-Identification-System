...
label_0 = Label(root, text="Criminal Face Identification System",width=87,font=("bold", 20),anchor=CENTER,bg="#386184",fg="white")
...
def viewdetail(a):
   ...
   for row in rows:
      ...
      label_c = Label(root, text=row[9],width=20,bg="#382273",font=("bold", 12),fg="white")
      label_c.place(x=1100,y=640)
   ...
   label_crime = Label(root, text="Crime :",bg="#382273",width=20,font=("bold", 12),fg='yellow')
  ...
  #  photo_l=Label(image=photo,width=250,height=250).place(x=690,y=400).pack()
   photo_l = Label(image=photo, width=250, height=250)
   photo_l.place(x=690, y=400).pack()
...
def mfileopen():
   ...
   photolbl = Label(image=photo, width=400, height=400)
   photolbl.image = photo
   photolbl.place(x=90, y=110)
...
if not images:
    ...
    # Proceed with loading and processing the images
    images_name = []
    for img in images:
        images_name.append(fr.load_image_file(os.path.join("images", img)))
    # Rest of your code for processing the images...
...
for img in images_name:
    face_encoding_list = fr.face_encodings(img)
    if face_encoding_list:
        encodings.append(face_encoding_list[0])
...
b2=Button(text="View Matching Records",width=30,height=2,command=View,bg='red',fg="white").place(x=165,y=590)
